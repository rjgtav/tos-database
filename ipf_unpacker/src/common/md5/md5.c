...
void
MD5_stringEx (char *string, char *result)
{
	...
	for (i = 0; i < 16; i++)
		sprintf(result + (i << 1), "%02x", (unsigned int) md5.digest[i]);
...
void
MD5_bufferEx (uint8_t *buffer, size_t size, char *result)
{
    struct MD5Context md5;
    int i;
...
char *
MD5_buffer (uint8_t *buffer, size_t size)
{
    char *result = NULL;
    ...
    MD5_bufferEx (buffer, size, result);
    return result;
}
